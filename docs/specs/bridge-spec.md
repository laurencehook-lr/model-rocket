...
- **REQ-001**: The launcher SHALL start the pinned unmodified official Claude Code binary by absolute path, bypass any same-named terminal command shim, and enforce the loopback route over user, project, local, and caller-supplied settings without bypassing managed enterprise policy.
...
- **REQ-017**: The launcher SHALL configure Claude Code to assume a 272,000-token context window for the custom GPT model.
- **REQ-018**: The launch-scoped routing settings file SHALL be readable only by its owner and SHALL be removed when Claude Code exits.
- **REQ-019**: The launcher SHALL force both standard proxy-bypass variables to cover the selected loopback host while preserving existing bypass entries.
